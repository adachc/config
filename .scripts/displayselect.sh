...
twoscreen() { # If multi-monitor is selected and there are two screens.
    ...
    mirror=$(printf "no\\nyes" | dmenu -b -i -p "Mirror displays?")
    # Mirror displays using native resolution of external display and a scaled
    # version for the internal display
    if [ "$mirror" = "yes" ]; then
        external=$(echo "$screens" | dmenu -b -i -p "Optimize resolution for:")
        ...
        primary=$(echo "$screens" | dmenu -b -i -p "Select primary display:")
        secondary=$(echo "$screens" | grep -v "$primary")
        direction=$(printf "left\\nright" | dmenu -b -i -p "What side of $primary should $secondary be on?")
...
morescreen() { # If multi-monitor is selected and there are more than two screens.
	primary=$(echo "$screens" | dmenu -b -i -p "Select primary display:")
	secondary=$(echo "$screens" | grep -v "$primary" | dmenu -b -i -p "Select secondary display:")
	direction=$(printf "left\\nright" | dmenu -b -i -p "What side of $primary should $secondary be on?")
	tertiary=$(echo "$screens" | grep -v "$primary" | grep -v "$secondary" | dmenu -b -i -p "Select third display:")
...
chosen=$(printf "%s\\nmulti-monitor\\nmanual selection" "$screens" | dmenu -b -i -p "Select display arangement:") &&
case "$chosen" in
